    ...
    <section className="bg-white rounded-2xl border border-[#E8E2D9] p-4 shadow-sm">
      ...
      {items.length === 0 ? (
        <p className="text-[#8B7E74] text-sm text-center py-4">
          Agregá ítems manualmente o escaneá el ticket
        </p>
      ) : (
        <ul className="space-y-3">
          {items.map((item) => (
            <li key={item.id} className="border border-[#E8E2D9] rounded-xl p-3 bg-[#FAF7F2]">
              <div className="flex items-center gap-2 mb-2">
                <input
                  ...
                  className="flex-1 text-sm bg-transparent focus:outline-none placeholder:text-[#8B7E74]"
                />
                <input
                  type="number"
                  value={item.price || ''}
                  ...
                  min="0"
                  step="0.01"
                  className="w-24 text-sm text-right bg-transparent focus:outline-none placeholder:text-[#8B7E74] font-medium"
                />
                <button
                ...
                <div className="flex flex-wrap gap-1.5">
                  {people.map((person) => {
                    ...
                    return (
                      ...
                      >
                        {person.name || '?'}
                        {assigned && item.assignedTo.length > 1 && (
